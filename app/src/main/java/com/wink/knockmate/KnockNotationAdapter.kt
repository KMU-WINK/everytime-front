...
class KnockNotationAdapter(val knockList: Array<KnockList>) :
    ...
    override fun onCreateViewHolder(
        ...
        val view = LayoutInflater.from(parent.context).inflate(R.layout.knock_notation_item,parent,false)
    ...
    override fun onBindViewHolder(holder: KnockNotationAdapter.CustomViewHolder, position: Int) {
        ...
        holder.nickname.text= Html.fromHtml(("<b>${knockList.get(position).nickname}</b>") + "님에게 노트가 도착했습니다.")
        holder.schedule.text = "${knockList.get(position).month}월 ${knockList.get(position).day}일 ${time} ${knockList.get(position).startTime}시 ${knockList.get(position).startMinute}분 - ${knockList.get(position).endTime}시 ${knockList.get(position).endMinute}분 "
